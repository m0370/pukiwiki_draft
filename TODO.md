...
- **ファイル**:
  ...
  - ❌ `lib/html.php`: 401, 405, 410, 451行にハードコード残存
...
- **未解決箇所** (`lib/html.php`):
  ...
  - 行451: `value="下書き保存"`
- **検証結果**: プラグイン層は完全に国際化対応済み。ライブラリ層の対応が必要。
...
8. ⚠️ **lib/html.phpの国際化** - 4箇所のハードコード残存（低優先度）
9. ⚠️ **XSS confirm()エスケープ** - 低リスクだが対応推奨（低優先度）

### 📋 未着手の項目
10. ○ **下書きの競合解決** - マルチユーザー環境での改善（中優先度）
11. △ **自動保存** - UX改善（低優先度）
12. △ **下書きのクリーンアップ** - 運用改善（低優先度）
13. △ **複数バージョン保存** - 高度な機能（低優先度）

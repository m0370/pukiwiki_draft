...
### 通常の「ページの更新」でも下書きは破棄されます

編集画面から「**ページの更新**」で保存すると、そのページの下書きは自動的に削除されます。
本文が本ページに入った時点で下書きの役目は終わっているためで、自動保存
（`autosave.js`）が作った下書きが公開後も残り続けることを防ぎます。
本文を空にしてページを削除した場合も、同じく下書きは削除されます。

一方、次の場合は下書きを削除しません。

- **衝突が検出されたとき**（他の人が先に更新していた場合）
- **パスワードエラーで編集画面に戻されたとき**
- **追記のとき**（`?cmd=add` からの追記や、ページ末尾へのコメント追加）

前の 2 つは保存が確定していないため、最後の 1 つは追記がページ全体を
書き上げる操作ではないためです。いずれも「1 行追記しただけで、そのページの
書きかけ全文が消える」といった損失を防ぐためのものです。
...
`draft/` ディレクトリは初回の下書き保存時に自動作成されるため、事前の作成は不要です。
...
**原因**: `draft/` ディレクトリの自動作成に失敗している、またはパーミッションが不足している
（失敗時は PHP のエラーログに `draft_write: failed to open ...` という診断情報が出力されます）
...
```bash
# パーミッション設定（draft/ は初回保存時に自動作成される）
chmod 777 wiki/
chmod 777 attach/
chmod 777 backup/
chmod 777 counter/

...
**注意**: 下書き一覧へのアクセスは制限されています:
- **読み取り専用モード** (`PKWK_READONLY = 1`) の場合はアクセス不可
- **編集認証が有効** (`$edit_auth = 1`) な場合、ログインユーザーのみアクセス可能
- 詳細は「アクセス制限」セクションを参照
...
#### 下書き一覧（`?cmd=draft`）へのアクセス制限

下書き一覧へのアクセスは以下の条件で制限されています:

**1. 読み取り専用モード**

`PKWK_READONLY = 1` の場合、下書き一覧にアクセスできません。
...
エラーメッセージ: `Prohibited by admin (READONLY mode)`

**2. 編集認証が有効な場合**

`$edit_auth = 1` の場合、ログインユーザーのみが下書き一覧にアクセスできます。
...
- **ログインしていない場合**: アクセス拒否
  - エラーメッセージ: `Authentication required to view draft list`
- **ログイン済みの場合**: アクセス可能

**3. 編集認証が無効な場合**

`$edit_auth = 0`（デフォルト）の場合:
- 誰でも下書き一覧にアクセス可能
- セキュリティリスクがあるため、認証を有効にすることを推奨

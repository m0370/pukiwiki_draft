...
**バージョン名**: Discard Draft on Save  
**バージョン**: 1.4.0  
**作成日**: 2025-11-22（最終更新: 2026-08-07）  
**対象**: PukiWiki 1.5.4 + Draft機能 v1.4.0  
**分析環境**: PHP 8.4.14

## 変更履歴（サマリ）
- **1.4.0 (2026-08-07)**: 通常保存で下書きが破棄されない問題を修正。`plugin/edit.inc.php` に `plugin_edit_discard_draft()` を新設し、通常保存パスと空本文によるページ削除パスの `page_write()` 成功直後から呼ぶ。衝突検出（digest 不一致）・パスワードエラーで編集フォームに戻る経路は `page_write()` より手前で return するため呼ばれない（書きかけを保護）。`lib/draft.php` が無い環境向けに `function_exists()` でガード。追記（`cmd=add`）は通常保存と同じ `page_write()` に到達するため、呼び出し側で `$add` を見て弾く（1 行追記で書きかけ全文が消えるのを防ぐ）。実運用サイトの棚卸しで、存在した下書き 23 件中 21 件（91%）が公開済みの残骸だったことが発端。
- **1.3.0 (2026-06-11)**: oncologynote.jp での運用改善を移植。`draft/` ディレクトリを初回保存時に自動作成（clone 直後に起動不能だった問題を根治）、`check_ticket()` を入力フィルタ済み `$post` 優先に堅牢化、`autosave.min.js` を追加して編集画面の読み込みを軽量化、`edit.inc.php` の軽微なクリーンアップ。
